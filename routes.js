...
routes.put("/items/:id", (req, res) => {
    pool.query("update shoppingcart set product=$2::text, price=$3::decimal, quantity=$4::int where id=$1::int", [req.params.id, req.body.product, req.body.price, req.body.quantity]).then(() => {
        pool.query("select * from shoppingcart").then((result) => {
            res.json(result.rows)
        }); 
    });
